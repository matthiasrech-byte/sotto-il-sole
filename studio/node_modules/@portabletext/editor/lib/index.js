...
`) {
  ...
  const $ = c(8), {
    text,
    isTrailing: t0
  } = props, isTrailing = t0 === void 0 ? !1 : t0, isInNewPipeline = useContext(NewPipelineContext), ref = useRef(null);
  let t1;
  $[0] !== isTrailing || $[1] !== text ? (t1 = () => `${text ?? ""}${isTrailing ? `
` : ""}`, $[0] = isTrailing, $[1] = text, $[2] = t1) : t1 = $[2];
  ...
  }, $[3] = getTextContent2, $[4] = t2) : t2 = $[4], useIsomorphicLayoutEffect(t2);
  let t3;
  ...
  defineBehavior({
    ...
    guard: ({
      ...
      const dragOrigin = event.originEvent.dragOrigin, dragSelection = getDragSelection({
        eventSelection: dragOrigin.selection,
        snapshot
      }), dropPosition = event.originEvent.position.selection, droppingOnDragOrigin = dragOrigin ? isDropTargetingDragOrigin(dropPosition, dragSelection, snapshot) : !1, draggingEntireBlocks = isSelectingEntireBlocks({
        ...snapshot,
        context: {
          ...snapshot.context,
          selection: dragSelection
        }
      }), draggedBlocks = getFragment({
        context: {
          ...snapshot.context,
          selection: dragSelection
        }
...
function createRelay() {
  ...
  return {
    ...
    on: (type, listener) => (listeners.set(type, [...listeners.get(type) ?? [], listener]), {
      unsubscribe: () => {
        const current = listeners.get(type) ?? [], index = current.indexOf(listener);
...
function createInternalEditor(config) {
  ...
  const subscriptions = [], editorActor = createActor(editorMachine, {
    ...
    on: (event, listener) => relay.on(event, (event2) => {
      switch (event2.type) {
        case "blurred":
        case "done loading":
        case "editable":
