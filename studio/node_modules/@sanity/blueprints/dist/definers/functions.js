import { validateDocumentFunction, validateEventFunction, validateFunction, validateMediaLibraryAssetFunction, validateQueueFunction, validateScheduledFunction, validateSyncTagInvalidateFunction, } from '../index.js';
...
const SCHEDULED_EVENT_KEYS = new Set(['minute', 'hour', 'dayOfWeek', 'month', 'dayOfMonth', 'expression']);
const QUEUE_EVENT_KEYS = new Set(['concurrency', 'fifo', 'dlq']);
 ...
 */
export function defineScheduledFunction(functionConfig) {
 ...
 */
export function defineQueueFunction(functionConfig) {
 ...
 */
export function defineEventFunction(functionConfig) {
    const functionResource = {
        ...defineFunction(functionConfig, { skipValidation: true }),
...
export function defineFunction(functionConfig, options) {
    ...
    if (options?.scopeType !== 'organization' && project)
        functionResource.project = project;
    if (lifecycle)
 ...
 */
function buildScheduledFunctionEvent(event) {
    return Object.fromEntries(Object.entries(event).filter(([key]) => SCHEDULED_EVENT_KEYS.has(key)));
}
/**
 ...
 */
function cronStringToExplicitEvent(cron) {
 ...
 */
function buildQueueFunctionEvent(event) {
    const defaultEvent = { concurrency: 1, fifo: true, dlq: true };
    if (typeof event === 'boolean') {
        return defaultEvent;
    }
    const userProvidedEvent = Object.fromEntries(Object.entries(event).filter(([key]) => QUEUE_EVENT_KEYS.has(key)));
    return {
        ...defaultEvent,
        ...userProvidedEvent,
    };
